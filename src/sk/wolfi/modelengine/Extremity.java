...
public class Extremity {
	...
	private String name;
	private Image image;
	private Point bodyPos; // the body position, and thus the extremity position is known only after the update loop.
	private boolean isHiddenBehindTorso;

	public Extremity(String name, int width, int height, String imgLocation, 
			int offsetExtrJointPosX, int offsetExtrJointPosY, int offsetBodyJointPosX, int offsetBodyJointPosY, boolean isHiddenBehindTorso) {
		this.name = name;
		this.width = width;
		this.height = height;
		this.offsetJointPos = new Point(offsetExtrJointPosX, offsetExtrJointPosY);
	...
	public void update(Point bodyPos) {
		...
		float x = this.bodyPos.getX() + this.offsetBodyJointPos.getX() - this.offsetJointPos.getX();
		float y = this.bodyPos.getY() + this.offsetBodyJointPos.getY() - this.offsetJointPos.getY();
		this.position = new Point(x, y);
		this.image.setCenterOfRotation(offsetJointPos.getX(), offsetJointPos.getY());
